...
test.describe.serial('S3 Bucket Operations', () => {
  ...
  async function createTestBucket(page: Page, prefix: string = 'test-bucket') {
  ...
  test('should delete bucket object successfully', async ({ page }) => {
    ...
    await page.waitForResponse(
      response =>
        response.url().includes('/api/object/list') && response.status() === 200
    )
